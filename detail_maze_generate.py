...
# 再帰深度の問題を回避するため（必要なら調整）
sys.setrecursionlimit(10000)

def load_mask(image_path, cell_size=2):
    """
    ...
    for y in range(grid_height):
        for x in range(grid_width):
            ...
            if maze[y][x]['E']:
                ...
                sample = orig_image.getpixel((min(mid_x, orig_image.width - 1), min(mid_y, orig_image.height - 1)))
...
def main():
    ...
    # セルサイズを2とすることで非常に細かいグリッド
    mask, grid_width, grid_height, cell_size = load_mask(image_path, cell_size=2)
    
    # 迷路生成開始点として、グリッドの中央付近など任意に選択
    ...
    maze_img.save("detail_maze.png")
    print("detail_maze.png saved.")
